...
New-Item -Path 'C:\' -Name 'Temp' # Fichier
New-Item -Path 'C:\' -Name 'Temp' -ItemType Directory  # Dossier
...
Get-Content -Path 'C:\Temp\File.txt' # Lecture du fichier
Set-Content -Path 'C:\Temp\File.txt' -Value 'Ligne de texte' # Remplacement du contenu du fichier
Add-Content -Path 'C:\Temp\File.txt' -Value 'Deuxième ligne' # Ajout de contenu dans le fichier
Clear-Content -Path 'C:\Temp\File.txt' # Suppression du contenu du fichier sans supprimer le fichier
...
Get-Item -Path 'HKLM:\SOFTWARE' | Get-Member | Select-Object -ExpandProperty TypeName -Unique
...
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne'

# Correspondance avec une expression régulière
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne \d+'

# Ne pas interpréter l'expression régulière
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne \d+' -SimpleMatch

# Ajouter un contexte (1 ligne avant, 0 ligne après)
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne' -Context @(1,0)

# Retourner toutes les lignes qui ne correspondent pas 
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne' -NotMatch

# La cmdlet renvoie un objet de type [Microsoft.PowerShell.Commands.MatchInfo]
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne' | Get-Member
Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne' | Select-Object -Property *
...
$MatchList = Get-Content -Path 'C:\Temp\File.txt' | Select-String -Pattern 'Ligne'
$MatchList.Matches
...
Get-Service | Select-Object -Property Name,StartupType,Description | Export-Csv -Path 'C:\temp\Services.csv' -NoTypeInformation -Delimiter ','
    # Import du fichier csv + filtre sur les 5 premiers services dont le type de démarrage est Automatic
Import-Csv -Path 'C:\temp\Services.csv' -Delimiter ',' | Where-Object StartupType -eq 'Automatic' | Select-Object -First 5
...
Get-Service | Select-Object -Property Name,StartupType,Description | ConvertTo-Html  | Out-File -FilePath 'C:\temp\services_raw.html'
...
$MyData = Get-Service | Select-Object -Property Name,StartupType,Description | ConvertTo-Html -Fragment
$colorTagTable = @{
...
Get-Service | Select-Object -Property Name,StartupType,Description | ConvertTo-Json | Out-File -FilePath 'C:\temp\services.json'
...
# Arrêter les processus lancés depuis le dossier C:\Program Files\Notepad++
    # Méthode 1
Get-WmiObject -Query 'SELECT * FROM Win32_PROCESS WHERE CommandLine Like "%C:\\Program Files\\Notepad++%"' | Invoke-WmiMethod -Name Terminate -Verbose
...
Format-Hex -path 'C:\temp\File.txt'
Get-Content -Path 'C:\temp\File.txt' | Format-Hex